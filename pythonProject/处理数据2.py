...
def main_function(group):
    fill = PatternFill(start_color='D9D9D9', end_color='D9D9D9', fill_type='solid')

    # 打开现有的工作簿
    wb_origin = openpyxl.load_workbook('/home/sf107/桌面/加班表格处理/模板/加班审批表模板.xlsx')  # ！！！
    ...
    wb_origin2 = openpyxl.load_workbook('/home/sf107/桌面/加班表格处理/模板/附件：2024年加班日志情况汇总表.xlsx')  # ！！！
    ws_origin2 = wb_origin2.active
    wb_origin3 = openpyxl.load_workbook('/home/sf107/桌面/加班表格处理/模板/单日加班超4小时申请汇总表模板.xlsx')  # ！！！
    ws_origin3 = wb_origin3.active
    wb_origin4 = openpyxl.load_workbook('/home/sf107/桌面/加班表格处理/模板/加班费申报表模板.xlsx')  # ！！！
    ws_origin4 = wb_origin4.active
    wb_origin5 = openpyxl.load_workbook(
        '/home/sf107/桌面/加班表格处理/模板/附件4：加班费审批表（上芬社区工作站）11月.xlsx')  # ！！！
    ws_origin5 = wb_origin5.active
    wb_origin6 = openpyxl.load_workbook(
        '/home/sf107/桌面/加班表格处理/模板/附件4：加班费审批表（上芬社区工作站）11月.xlsx')  # ！！！
    ...
    df1 = pd.DataFrame()
    # 指定目录和通配符
    directory = '/home/sf107/桌面/加班表格处理/'
    pattern = '加班申请*.xlsx'
    # 获取匹配的文件列表
    matches = []
    ...
    # 结果的表格
    columns = ['序号', '人员类别', '姓名', '="加班日期"&CHAR(10)&"附时间段"', '加班事由', '加班类型', '加班费倍率',
               '学历', '工资基数', '加班费基数/小时', '加班时长（小时）', '加班费金额（元）', '个人小计']
    columns2 = ['序号', '人员类别', '姓名', '="加班日期"&CHAR(10)&"附时间段"', '加班事由', '加班类型', '加班费倍率',
                '学历', '工资基数', '加班费基数/小时', '加班时长（小时）', '加班费金额（元）', '个人小计', '备注', '画×',
                '报加班费还是给调休', '本次加班获得调休时长', 'data_id_x']
    ...
    def handel_df_final(begin_num_final, letter_final, flag):
        ...
        result_handeled['类别（加班费使用）'] = Categorical(result_handeled['类别（加班费使用）'],
                                                          categories=['社区专职', '劳务派遣（天域）', '劳务派遣（天悦）',
                                                                      '劳务派遣（翰林晟）', '党务', '其他'], ordered=True)
    ...
    if len(df20) > 0:
        df20['是否应急值班'] = df20.apply(lambda row:
                                          '工作日值班' if row['加班类型'] == '工作日加班' and row[
                                              '加班类别'] == abc else
                                          '节假日值班' if row['加班类型'] == '周末加班' and row['加班类别'] == abc else
                                          '法定节假日值班' if row['加班类型'] == '法定节假日加班' and row[
                                              '加班类别'] == abc else
                                          '否',
                                          axis=1
                                          )
        ...
        df22 = df20.groupby(
            ['加班日期', '加班类型', '加班类别', '具体事由', '是否应急值班', '调休或加班费']).size().reset_index(
            name='人数')

        df23 = df20.groupby(['加班日期', '加班类型', '加班类别', '具体事由', '是否应急值班', '调休或加班费'])[
            '姓名'].agg('、'.join).reset_index()
        print(df23)
        ...
        merged_df['加班类别'] = merged_df['加班类别'].replace(abc, '常规性值班工作')
        merged_df['日期和总人数'] = merged_df.apply(lambda row: f'{row["加班日期"].month}月{row["加班日期"].day}日',
                                                    axis=1)
        merged_df = merged_df.rename(columns={'具体事由': '加班事项'})
        merged_df = merged_df.drop('加班日期', axis=1)
        merged_df = merged_df.reindex(
            columns=['日期和总人数', '加班类型', '加班类别', '加班事项', '是否应急值班', '调休或加班费', '人数',
                     '姓名'])
    ...
    ws_origin2['H100'] = "申报加班费总时长：" + str(
        df20[df20['报加班费还是给调休'].str.contains('加班费')]['加班计算小时数'].sum())
    ...
    # 设置列
    df31 = df300[
        ['序号', '姓名', '="加班日期"&CHAR(10)&"附时间段"', '加班事由', '加班类型', '加班费倍率', '加班时长（小时）']]
    # 填充数据 单日加班超4小时申请汇总表
    cc = 3
    ...
    df41 = df40.groupby(['加班类型', '姓名', '人员类别', '加班费申报表顺序', '审批人（加班费使用）', '工资基数'])[
        ['加班计算小时数', '加班费金额']].sum().reset_index()
    ...
    df42['加班类型'] = Categorical(df42['加班类型'], categories=['工作日加班', '公休日加班', '法定节假日加班'],
                                   ordered=True)
    ...
    for k5 in range(1, len(df50) + num5 + 1):
        ...
        if ws_origin5[f'C{k5 + b5}'].value != ws_origin5[f'C{k5 + b5 - 1}'].value:
            if ws_origin5[f'C{k5 + b5 - 1 - count5}'].value is not None and ws_origin5[
                f'C{k5 + b5 - 1 - count5}'].value != '':
    ...
    for k6 in range(1, len(df60) + num6 + 1):
        ...
        if ws_origin6[f'C{k6 + b6}'].value != ws_origin6[f'C{k6 + b6 - 1}'].value:
            if ws_origin6[f'C{k6 + b6 - 1 - count6}'].value is not None and ws_origin6[
                f'C{k6 + b6 - 1 - count6}'].value != '':
    ...
    # 将工作簿保存为 Excel 文件
    wb.save("/home/sf107/桌面/加班表格处理/没用/处理完成2.xlsx")
    wb1.save("/home/sf107/桌面/加班表格处理/没用/result.xlsx")
    wb_temp.save("/home/sf107/桌面/加班表格处理/没用/result_temp.xlsx")
    wb_origin.save(f"/home/sf107/桌面/加班表格处理/分组处理结果/{group}/原来的加班费审批表{current_month}月.xlsx")  # ！！！
    wb_origin2.save(
        f"/home/sf107/桌面/加班表格处理/分组处理结果/{group}/（新模板）{current_year}年{current_month}月加班日志汇总表.xlsx")  # ！！！
    wb_origin3.save(f"/home/sf107/桌面/加班表格处理/分组处理结果/{group}/单日加班超4小时申请汇总表.xlsx")  # ！！！
    wb_origin4.save(f"/home/sf107/桌面/加班表格处理/分组处理结果/{group}/附件2：加班费申报表（上芬社区工作站）.xlsx")  # ！！！
    wb_origin5.save(
        f"/home/sf107/桌面/加班表格处理/分组处理结果/{group}/附件4：加班费审批表（上芬社区工作站）{current_month}月.xlsx")  # ！！！
    wb_origin6.save(f"/home/sf107/桌面/加班表格处理/分组处理结果/{group}/{current_month}月领导画×表.xlsx")  # ！！！

    # subprocess.run(['python', '处理调休表.py'])


for group in ['综合组', '网格组', '处置组']:
    main_function(group)